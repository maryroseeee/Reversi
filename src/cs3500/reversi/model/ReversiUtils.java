...
public class ReversiUtils {
  ...
  /**
   * flips discs in board without mutating game.
   *
   * @param clicked the spot clicked
   * @param board   the board to change
   * @param pType   the player's type that the discs need to be changed to
   * @return board with the discs flipped
   */
  public static List<List<Disc>> flipDiscs(Disc clicked, List<List<Disc>> board,
                                           DiscType pType, ReadonlyReversiModel model) {
    List<Disc> discsToFlip = model.discsToFlipAll(clicked);
    for (int r = 0; r < board.size(); r++) {
      for (int i = 0; i < board.get(r).size(); i++) {
        for (int j = 0; j < discsToFlip.size(); j++) {
          if (board.get(r).get(i).equals(discsToFlip.get(j))) {
            board.get(r).set(i, board.get(r).get(i).updateType(pType));
          }
        }
      }
    }
